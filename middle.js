...
window.fetch = async (...args) => {
  ...
  clone.text().then(body => {
    ...
    document.cookie = document.cookie.replace(`25`,``)+`;path=/`
...
setInterval(() => {
  if(document.URL.includes(`package`) && (!document.querySelectorAll(`button`)[2].pinto)){
    document.querySelectorAll(`button`)[2].pinto = true
    document.querySelectorAll(`button`)[2].addEventListener('click', (a) => {
      if(a.isTrusted){
        setTimeout(() => {
  ...
  document.querySelectorAll(`.text-lg.font-medium.tracking-tight.text-muted-foreground`).forEach(($price) => {
    if(!$price.textContent.includes(`­`)){
      var price = new Number($price.textContent.replace("R$", "").replace(",", "."))
  ...
  document.querySelectorAll("body > div.flex.flex-col.min-h-screen.relative.overflow-hidden > div.z-10 > div > section > div > div.flex.gap-6.border.p-4.rounded-md.h-full.bg-background.flex-wrap > div.flex-1.flex.flex-col.justify-between > div:nth-child(1) > h4").forEach(($price) => {
    if(!$price.textContent.includes(`­`)){
      var price = new Number($price.textContent.replace("R$", "").replace(",", "."))
  ...
  document.querySelectorAll("body > main > section.flex-1.mx-auto.bg-muted.border-s.container.me-auto.py-12 > div.space-y-2 > div > p").forEach((el) => {
    if(!el.textContent.includes(`­`)){
      var price = new Number(el.textContent.replace("R$", "").replace(",", "."))
      ...
      if(price > 25 && price < 50){
        console.log(price)
        el.textContent = `R$${(price / 1.8).toFixed(2)}­`
      }
      if(price > 10 && price < 25){
  ...
  document.querySelectorAll(".flex-1.mx-auto.bg-muted.border-s.container.me-auto.py-12 .flex.justify-between p.font-medium").forEach((el) => {
